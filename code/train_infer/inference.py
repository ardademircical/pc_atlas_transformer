...
def run_inference(model, train_population):
    ...
    for pop in prostate_populations:
        if pop == train_population:
            continue

        # Load the CSV file for each inference population
        inference_path = f"nn_files/train_infer_data/df/{train_population}/inference_df_labeled_{pop}.csv"
        inference_df = pd.read_csv(inference_path)
        inference_pca_embeddings = torch.load(f"nn_files/train_infer_data/pt/{train_population}/inference_pca_vectors_{pop}.pt")
    ...
    results_path = f"inference_results/inference_results_{train_population}.json"
